<!---
	...
	--->
	<!--- Gregory Alexander modified on 12/16/208 in order to work with Kendo themes. --->
<cfcomponent output="false">
	<cffunction name="init" access="public" hint="This function initializes all of the variables needed for the component." output="false">
		...
		<cfset setConfig(createObject("component", "coldfishconfig").init(file))/><!---Gregory added the file argument within the init. Used to be 'init()'. I changed this to 'init(file)' in order to pull in other xml files depending upon the current theme. --->
        <cfreturn this/>
    </cffunction>
