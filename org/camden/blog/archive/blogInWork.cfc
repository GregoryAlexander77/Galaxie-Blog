...
<a href="#rooturl#unsubscribe.cfm?email=#email#&amp;token=#token#">#rooturl#unsubscribe.cfm?email=#email#&amp;token=#token#</a>
	...
	<cffunction name="renderEntry" access="public" returnType="string" output="false"
		...
		<!--- 
		Gregory revised this to allow scripts and css files to be saved into the database. I also changed this to remove paragraph formatting that was being applied to the scripts and css style sheets. 
q		--->
		...
		<!--- Gregory's code. I need to allow users to include script tags in the blog entry. My current provider, Hostek, has 'enable global script protection' turned on creating an invalidTag response when submitting code that includes scripts. The following code injects the script tags if the user entered in the folllowing tag: <attachScript>> </attachScript> --->
			<!--- Append the script to the arguments.string. --->
			<cfset attachScript = replaceNoCase(arguments.string, "attachScript", "script", "all")>	
		</cfif>
		...
		<cfif reFindNoCase(tbRegex,arguments.string)>
			...
			<cfloop condition="counter gte 1">
				...
				<cfif arrayLen(textblock.pos) is 2>
					...
					<!--- Apply paragraph formatting to the newstring content if necessary. --->
					<cfif arguments.ignoreParagraphFormat>
						<cfset arguments.string = replaceNoCase(arguments.string, textblockTag, newContent)> 
					<cfelse>
						<cfset arguments.string = xhtmlParagraphFormat(replaceNoCase(arguments.string, textblockTag, newContent))> 
					</cfif>
				</cfif>
		...
		<cfloop item="cfc" collection="#variables.renderMethods#">
			...
			<cfelse>
				<cfset newstring = xhtmlParagraphFormat(newstring) />
			</cfif>
		...
		<cfif len(arguments.enclosure) and listFindNoCase("gif,jpg,png", listLast(arguments.enclosure, "."))>
			...
			<!--- Gregory renamed the autoImage div to entryImage. I did not see any autoImage classes elsewhere and want the name to be related to a blog post. I am also constraining the image with .css. --->
