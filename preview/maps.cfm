				...
				function getMap() {
					try {
						...
						map.events.add('ready', function () {
							...
							<!--- Create our waypoints Note the GeoJSON objects have been switched from Bing Maps to Azure Maps. Now we are using longitude first then latitude instead of the other way around. --->
						<cfloop from="1" to="#arrayLen(Data)#" index="i"><cfoutput>
							...
							map.setCamera({
								...
								<!--- Padding will essentially zoom out a bit. The default is 50, I am using 100 as I want the destinations on the map to be clearly shown. However, this will cause errors if the map size exceeds the bounds so I am not using it with small map previews. padding: 100 --->
							});
            ...
            function loadMapScenario() {
				...
				<cfif isDefined("URL.thumbnail") and URL.thumbnail eq 'true'>
					...
					<cfloop from="1" to="#arrayLen(Data)#" index="i"><cfoutput>
						...
						<!--- Set the element in which the itinerary will be rendered directionsManager.setRenderOptions({ itineraryContainer: document.getElementById('printoutPanel') }); --->
