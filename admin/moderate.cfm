...
<cfif structKeyExists(url, "approve")>
	...
	<cfset subject = getResourceBundle("commentaddedtoblog") & ": " & application.blog.getProperty("blogTitle") & " / " & getResourceBundle("entry") & ": " & entry.title>
...
<body id="blogcommentmail" style="font:10pt Arial,sans-serif;padding: 10px;">
	<table cellspacing=0>
		<tr id="header">
			<td colspan=2>Comment Added to <a href="#application.blog.makeLink(entry.id)###c#c.id#">#htmlEditFormat(application.blog.getProperty("blogTitle"))# : #entry.title#</a></td>
		</tr>
...
#paragraphformat2(htmlEditFormat(c.comment))#
			...
			<td id="commentor" valign=top style="width:25%;background-color: ##edf0c9;height:100%">
				<div id="avatar" style="text-align: center;margin:30px 0 0 0;padding:20px 0 20px 0;width: 100%;height: 100%;">
					...
					<div id="commentorname" style="text-align: center;padding:20 0 20px 0;"><cfif len(c.website)><a href="#c.website#"></cfif>#c.name#<cfif len(c.website)></a></cfif></div>
				</div>
			</td>
		</tr>
		<tr><td colspan=2 style="height:10px"></td></tr>
