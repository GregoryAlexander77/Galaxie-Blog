    ...
    <script>
		...
		.on('upload-success', (file, response) => {
			<!--- The server is returning location and mediaId in a json object. We need to extract these. alert(response.status) // HTTP status code alert(response.body.location) // The full path of the file that was just uploaded to the server alert(response.body.mediaId) // The MediaId value saved to the Media table in the database. alert(response.body.fileContent) // The content of the webvtt file --->
