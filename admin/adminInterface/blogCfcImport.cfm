	<table>
		<tr>
			...
			<td>
				...
				<p>Locate the <cfoutput>#application.baseUrl#</cfoutput>/common/data/generateBlogCfcDataFiles.cfm template in this installation and upload the entire data directory and files to a server that has access to your original BlogCFC or Galaxie Blog database. You will need to manually modify the following cfsetsettings at the very top of the template to communicate to your orginal database: </p>
				<ol>
					<li>If you don't want to generate the files to the default directory (files), set the destination path on the original database server where you want the WDDX files to be generated to.</li>
					...
					<li>The generateBlogCfcDataFiles.cfm template has been tested against all of the Galaxie Blog 1x versions and BlogCFC version 6. You may need to modify this template if you are running a version less than BlogCFC 6. To the best of my recollection, BlogCFC version 5.98 is missing a single database column and you will have to set the following column output to '' to bypass the error and properly generate the WDDX files ('' as MissingColunm).</li>
		...
		<tr>
			<td colspan="2">
				<p>This import template has been successfully tested importing the data from Gregory's Blog at www.gregoryalexander.com/blog/. Gregory's Blog has hundreds of posts and each post has an enclosure image. If you have more than 150 posts and recieve a query time-out error, you may want to modify this template and change how many records are run at a single given time. Alternatively, you can adjust the number of records in the WDDX files by changing the SQL in the generateBlogCfcDataFiles.cfm template.</p>
