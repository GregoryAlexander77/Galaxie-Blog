...
<script>
	...
	<!--- Set a cookie indicating the screen size. We are going to use this to determine what interfaces to use when the the screens are narrow. (setCookie(name,value,days)) --->
	...
	function setScreenProperties(){
		...
		<!--- alert('windowWidth:' + windowWidth); alert('breakpoint:' + breakpoint); --->
		
		<!--- Notes: 1) This may be converted into media queries in an upcoming version. 2) This function will be invoked twice. Once upon page load, and then again when the body detects a resize. 3) This was designed to chose the appropriate image and maximize the size of the background image when the desktop or tablet has a wide screen size. The contentWidth applies to the header, and the outer container that holds the mainContainer and sidebar container elements. Using contentWidth of 66% looks good when the screen width is at least 1600x900, which is the size of a 20 inch monitor. The 66% setting looks great with a 20 inch monitor. 80% works with 1280x768, which is a 19 inch monitor or a 14 Notebook. I am adjusting the contentWidth via javascript to ensure proper rendering of the page. --->
		...
		if (windowWidth <= breakpoint){
			...
			$("#sidebarPanel").show(); 
			<!--- Display the hamburger in the menu (the 5th node). $(".k-menu > li:eq(4)").show(); --->
		} else {
		...
		<!--- Set the fixed nav width. We are going to add additional padding to the contentWidth. document.documentElement.style.setProperty('--fixedNavWidth', mainContainerWidth + contentPaddingPixelWidth); --->
		
		<!--- Double check and make sure that the main container and header width matches (it won't match right now as the padding that I had used increases the content size). I'll fix in the next version. Get all of the styles. --->
	...
	function scrollToTop(){
		...
		<!--- Close the menu that is calling this function (I would do it in the menu, but I can only call a simple function from there). Get a reference to the menu widget --->
	...
	<!--- Lazy loading images and media. Define a callback function to add a 'shown' class into the element when it is loaded --->
