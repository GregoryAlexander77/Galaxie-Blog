			...
			controller: {
				...
				deleteItem: function(value, item) {
					return $.ajax({
						...
						<!--- REST IN PEACE MY DEAR TOBY (THE GREATEST DOG EVER DIED RIGHT AFTER I WROTE THIS CODE). Pass the needed data. --->
						data: {
							<!--- Pass the fontId to the cfc on the back end. Note: the fontId is not in the grid, but it is within the data that is passed to the grid. Anything coming from the json string that is used to load data is available. --->
							csrfToken: <cfoutput>'#csrfToken#'</cfoutput>,
							fontId: value.FontId
						},
						dataType: "json",
