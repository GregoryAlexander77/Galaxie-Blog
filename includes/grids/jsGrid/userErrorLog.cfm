<!doctype html>
<cfsilent>
<cfset gridName = "userErrorLog">
<!--- Don't show the edit button in the grid --->
<cfset showEditButton = false>
<!---
Other than the grid name, this grid is identical to the visitor log grid. I need to have a separate template in order to render it properly in a new window. You can view this in a couple of ways. Typically, the grid displays all records, you can limit it to the ipAddressId by passing in the optional ipAddressId.
The arguments are: createAdminInterfaceWindow(59[,ipAddressId]) (createAdminInterfaceWindow(Id, optArgs, otherArgs, otherArgs1)
--->
<cfparam name="pageTitle" default="" type="string">
<cfset getUrl = application.baseUrl & '/common/cfc/ProxyController.cfc?method=getErrorLogForGrid&gridType=jsGrid'>
...
<head><cfoutput>
	...
	<p>All columns are sortable and searchable. Click on the ID to see the error detail. To search, enter the search term on top of the column and click the search link at the right of the page. Note: to conserve resources, this query only returns the last 10000 rows of data. However, all of data inside the retention period specified in the blog options interface is intact in the Error table in the database if you need to perform further manual analsis.</p>
			...
			fields: [
				{ 
					...
					itemTemplate: function(value, item) {
					  return '<a href="javascript:createAdminInterfaceWindow(60, ' + item.ErrorLogId + ');">' + value + '</a>';
					}
				},
				{ 
					name: "ErrorMessage", 
					...
					title: "Error Message",
					editing: false,
					width: (pageWidth*(<cfif session.isMobile>75<cfelse>55</cfif>/100)),
				},
