...
<head><cfoutput>
	...
	<p>All columns are sortable and most are searchable. To search, enter the search term on top of the column and click the search link at the right of the page. To change a tag, click on a row, enter text, and click on the update checkmark on the right of the page.</p>
...
<form id="tagsGridForm" action="#" method="post" data-role="validator">
	<!--- Pass the csrfToken --->
	...
	<table align="center" class="k-content tableBorder" width="100%" cellpadding="2" cellspacing="0" border="0">
	  ...
	  <tr valign="middle" height="30px">
		<td align="left" width="75%" class="<cfoutput>#thisContentClass#</cfoutput>">
			<button id="newTag" name="newTag" class="k-button k-primary" type="button" onClick="createAdminInterfaceWindow(49,'#: id #', '#: value #');">Create New Tag</button>
		</td>
	  </tr>
	  <cfsilent>
	...
	$(function() {
		...
		$("#<cfoutput>#gridName#</cfoutput>").jsGrid({
			...
			pageSize: 15,
			pageButtonCount: 5,
			deleteConfirm: "Do you really want to delete this tag?",
			controller: {
				loadData: function (filter) {
					...
					<!--- Extract any errors. This is a new jQuery promise based function as of jQuery 1.8. --->
						...
						if (jqXHR.status === 403) { 
							...
							$.when(kendo.ui.ExtAlertDialog.show({ title: "Error while consuming the getTags function", message: error, icon: "k-ext-error", width: "<cfoutput>#application.kendoExtendedUiWindowWidth#</cfoutput>" }) // or k-ext-error, k-ext-information, k-ext-question, k-ext-warning.  You can also specify height.
								).done(function () {
								<!--- Do nothing --->
							});
						}//...if (jqXHR.status === 403) { 
				...
				updateItem: function(value, item) {
					...
					<!--- Extract any ajax errors. This is a new jQuery promise based function as of jQuery 1.8. --->
						...
						if (jqXHR.status === 403) { 
							...
							$.when(kendo.ui.ExtAlertDialog.show({ title: "Error saving tag", message: error, icon: "k-ext-warning", width: "<cfoutput>#application.kendoExtendedUiWindowWidth#</cfoutput>", height: "125px" }) // or k-ext-error, k-ext-question
							).done(function () {
								<!--- Do nothing --->
							});
						}//...if (jqXHR.status === 403) {
				...
				deleteItem: function(value, item) {
					return $.ajax({
						...
						data: {
							csrfToken: <cfoutput>'#csrfToken#'</cfoutput>,
							<!--- Pass the tagId to the cfc on the back end. Note: the commentId is not in the grid, but it is within the data that is passed to the grid. Anything coming from the json string that is used to load data is available. --->
							tagId: value.TagId
						},
						dataType: "json",
						cache: false,
					...
					<!--- Extract any errors. This is a new jQuery promise based function as of jQuery 1.8. --->
						...
						if (jqXHR.status === 403) { 
							...
							<!--- The full response is: jqXHR.responseText, but we just want to extract the error. --->
							$.when(kendo.ui.ExtAlertDialog.show({ title: "Error while consuming the deleteTagViaGrid function", message: error, icon: "k-ext-error", width: "<cfoutput>#application.kendoExtendedUiWindowWidth#</cfoutput>" }) // or k-ext-error, k-ext-information, k-ext-question, k-ext-warning.  You can also specify height.
								).done(function () {
								<!--- Do nothing --->
							});
						}//...if (jqXHR.status === 403) { 
