...
<head><cfoutput>
	...
	<p>All columns are sortable and searchable. Click on the ID to see the error detail. To search, enter the search term on top of the column and click the search link at the right of the page.</p>
		...
		$("#<cfoutput>#gridName#</cfoutput>").jsGrid({
			...
			controller: {
				loadData: function (filter) {
					...
					return $.ajax({
						type: "GET",
						url: "<cfoutput>#application.baseUrl#</cfoutput>/common/cfc/ProxyController.cfc?method=getSearchQueryForGrid&gridType=jsGrid&csrfToken=<cfoutput>#csrfToken#</cfoutput>", 
						data: filter,
						dataType: "json"
					...
					<!--- Extract any errors. This is a new jQuery promise based function as of jQuery 1.8. --->
						...
						if (jqXHR.status === 403) { 
							...
							$.when(kendo.ui.ExtAlertDialog.show({ title: "Error while consuming the getSearchQueryForGrid function", message: error, icon: "k-ext-error", width: "<cfoutput>#application.kendoExtendedUiWindowWidth#</cfoutput>" }) // or k-ext-error, k-ext-information, k-ext-question, k-ext-warning.  You can also specify height.
								).done(function () {
								<!--- Do nothing --->
							});
						}//...if (jqXHR.status === 403) { 
