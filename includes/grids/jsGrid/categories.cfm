...
<head><cfoutput>
	...
	<p>All columns are sortable and most are searchable. To search, enter the search term on top of the column and click the search link at the right of the page. To change a category, click on a row, enter text, and click on the update checkmark on the right of the page.</p>
...
<form id="categoriesGridForm" action="#" method="post" data-role="validator">
	<!--- Pass the csrfToken --->
	...
	<table align="center" class="k-content tableBorder" width="100%" cellpadding="2" cellspacing="0" border="0">
	  ...
	  <tr valign="middle" height="30px">
		<td align="left" width="75%" class="<cfoutput>#thisContentClass#</cfoutput>">
			<button id="newCategory" name="newCategory" class="k-button k-primary" type="button" onClick="createAdminInterfaceWindow(12,'#: id #', '#: value #');">Create New Category</button>
		</td>
	  </tr>
	  <cfsilent>
		...
		$("#<cfoutput>#gridName#</cfoutput>").jsGrid({
			...
			controller: {
				loadData: function (filter) {
					...
					return $.ajax({
						type: "GET",
						url: "<cfoutput>#application.baseUrl#</cfoutput>/common/cfc/ProxyController.cfc?method=getCategoriesForGrid&gridType=jsGrid&csrfToken=<cfoutput>#csrfToken#</cfoutput>",
						data: filter,
						dataType: "json"
					...
					<!--- Extract any errors. This is a new jQuery promise based function as of jQuery 1.8. --->
						...
						if (jqXHR.status === 403) { 
							...
							$.when(kendo.ui.ExtAlertDialog.show({ title: "Error while consuming the getCategories function", message: error, icon: "k-ext-error", width: "<cfoutput>#application.kendoExtendedUiWindowWidth#</cfoutput>" }) // or k-ext-error, k-ext-information, k-ext-question, k-ext-warning.  You can also specify height.
								).done(function () {
								<!--- Do nothing --->
							});
						}//...if (jqXHR.status === 403) { 
				...
				updateItem: function(value, item) {
					...
					<!--- Extract any ajax errors. This is a new jQuery promise based function as of jQuery 1.8. --->
						...
						if (jqXHR.status === 403) { 
							...
							$.when(kendo.ui.ExtAlertDialog.show({ title: "Error saving category", message: error, icon: "k-ext-warning", width: "<cfoutput>#application.kendoExtendedUiWindowWidth#</cfoutput>", height: "125px" }) // or k-ext-error, k-ext-question
							).done(function () {
								<!--- Do nothing --->
							});
						}//...if (jqXHR.status === 403) {
				...
				deleteItem: function(value, item) {
					return $.ajax({
						type: "post",
						url: "<cfoutput>#application.baseUrl#</cfoutput>/common/cfc/proxyController.cfc?method=deleteCategoryViaJsGrid",
						<!--- Pass the needed data. Here we need to see if the comment was approved, and pass the postId to update the database on the back end. --->
						data: {
							csrfToken: <cfoutput>'#csrfToken#'</cfoutput>,
							<!--- Pass the categoryId to the cfc on the back end. Note: the commentId is not in the grid, but it is within the data that is passed to the grid. Anything coming from the json string that is used to load data is available. --->
							categoryId: value.CategoryId
						},
						dataType: "json",
						cache: false,
					...
					<!--- Extract any errors. This is a new jQuery promise based function as of jQuery 1.8. --->
						...
						if (jqXHR.status === 403) { 
							...
							<!--- The full response is: jqXHR.responseText, but we just want to extract the error. --->
							$.when(kendo.ui.ExtAlertDialog.show({ title: "Error while consuming the deleteCategoryViaGrid function", message: error, icon: "k-ext-error", width: "<cfoutput>#application.kendoExtendedUiWindowWidth#</cfoutput>" }) // or k-ext-error, k-ext-information, k-ext-question, k-ext-warning.  You can also specify height.
								).done(function () {
								<!--- Do nothing --->
							});
						}//...if (jqXHR.status === 403) { 
			...
			fields: [
				{ 
					...
					title: "Category",
					editing: true,
					width: (pageWidth*(<cfif session.isMobile>80<cfelse>35</cfif>/100)),
				},
