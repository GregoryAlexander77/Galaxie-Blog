...
<script>
	$(document).ready(function() {
		...
		<!--- Mvvm logic. Create an observable model. The model will be used to bind this template to the body of the page. It is not referenced in the UI. --->
	    var captchaTextUiModel = kendo.observable({
			<!--- This was a bit tricky when I first coded something like this years ago. The captchaTextUi is a js object that references the datasource. In the datasource, I have to have a name for the datasource in order to refresh the datasource when I want the contents dynamically changes (in this case, on the reloadCaptcha js method). The captchaTextObj is referenced as the 'source' in the tbody bind. --->
		...
		<!--- Validation. NEW Was the post button clicked? We need to know this as the user can fill out the form, and if it validated, it will go ahead and post the form prior to the user clicking on the post button. This is due to having two exit points out of the validater. --->
		...
		addCommentSubmit.on('click', function(e){      
			...
			sessionStorage.setItem("captchaValidatedAttempts", 0);
			<!--- Note: when using server side logic, this function may not post the data to the server due to the time required to return the validation from the server. If the form has been successfully validated. --->
		...
		addCommentFormValidator = $("#addCommentSubscribe").kendoValidator({
			...
			rules: {
				<!--- Name of custom rule. This can be any name, but I typically put the name of the field and a verb to indicate what I am enforcing ('nameIsRequired'). Note: if you just want to check to see if something was entered you can specify 'required' in the form element. --->
				commenterName:
				function(input){
				...
				<!--- Email and website are aready validated on the client side. Both fields can be validated with the HTML 5 specification. This rule is quite different as it relies upon server side processing. I used https://www.telerik.com/blogs/extending-the-kendo-ui-validator-with-custom-rules as an example to build this. --->
				captcha: 
					function(input) {
				...
				if (element.val() != getCapthchaValidatedValue() || getCaptchaValidatedAttempts() <= 5){
					...
					$.ajax({
						...
						success: function(data) { // The `data` object is a boolean value that is returned from the server.
							var captchaValidated = getCapthchaValidated();
							if (data){
								<!--- debugging alert('Yes!'); Set the value on the cache object so that it can be referenced in the next validation run. Note: sessionStorage can only store strings. --->
