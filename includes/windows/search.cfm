...
<!--- Limit to 21 records (I want odd numbers due to the card grid) --->
...
<script>
	...
	$(document).ready(function() {
		...
		<!--- Validation !!! Note on the validators, all forms need a name attribute, otherwise the positioning of the messages will not work. Also data attributes that are dash separated become camel cased when retrieved using jQuery. --->
		searchFormValidator = $("#search").kendoValidator({
			<!--- Set up custom validation rules --->
			rules: {
				<!--- Search term Name of custom rule. This can be any name, but I typically put the name of the field and a verb to indicate what I am enforcing ('nameIsRequired'). Note: if you just want to check to see if something was entered you can specify 'required' in the form element. --->
				searchTermRequired:
				function(input){
